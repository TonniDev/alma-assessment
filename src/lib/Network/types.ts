...
export interface IRequestConfig<PayloadFormat = unknown, ParamsFormat = Record<string, unknown>> {
...
export type IAxiosService<
  ResponseFormat = unknown,
  PayloadFormat = unknown,
  ParamsFormat = Record<string, unknown>,
> = INetworkService<AxiosResponse<ResponseFormat>, PayloadFormat, ParamsFormat>;

export interface IRequestResponse<ResponseType = unknown> {
  status: number;
  data: ResponseType;
}

export type TSingleResponse<ResponseType> = IRequestResponse<ResponseType>;

export type TRepositoryResponse<Response> = Promise<TSingleResponse<Response>>;
